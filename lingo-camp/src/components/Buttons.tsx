    ...
    return (
        <div className="mt-4 inset-x-0 bottom-0 space-y-2">
            <Button className="w-full" type="primary" onClick={()=>{
                pushOne(completedArray.value, markdownSrc.value.replace("/asset/Basic JavaScript/", ""))
            }}>
                Test
            </Button>
            <Button className="w-full" onClick={()=>{
                let result = completedArray.value.filter(item => item !== markdownSrc.value.replace("/asset/Basic JavaScript/", ""))
